...
8. Search Tool 只能调用 `SearchUseCase`。Runtime 要求回退时，由 Agent Server Adapter 调用同一个 Direct Search Use Case，返回 `AGENT_TO_DIRECT_FALLBACK`，不绕过 Search Context。
