...
Agent Phase 3 已经完成，Phase 4 的行为事实与实时特征两个深化切片也已完成。Agent Runtime 后续演进中的 AR-1“取消语义闭环”、AR-2“完整消息事件与多轮历史”、AR-3“Checkpoint、pending Tool 与恢复协议”和 AR-4“Mutating Tool 副作用账本”已完成，AR-5“Steer Queue/Drain”为下一步，实施记录见[模块路线](../../platform/agent-runtime/ROADMAP.md)。OpenAI-compatible Adapter 现兼容标准 `message.content` 与 LongCat 的 `message.reasoning_content`；本地已用 LongCat-2.0 跑通一次模型 → Agent → Search Tool → Web 的真实功能验收，并观测到 Provider 返回的 Token usage。默认固定评测仍使用可复现的确定性 Provider，这次验收不冒充质量或成本基线。HITL、Handoff、子 Agent、MCP、Steer Queue/Drain、上下文压缩、流式 Push 和完整 OpenTelemetry 仍未完成；每个真实写 Tool 的外部状态查询或补偿也必须随具体集成单独验收。Ark-Leto 反向核对矩阵见 [ADR-006](../adr/ADR-006-agent-reliability-fencing-outbox-shadow.md)。
